...
//  Messenger
//
//  Created by Katharos on 05/12/2022.
//
        ...
        DatabaseManager.shared.getAllMessagesForConversation(with: id) { [weak self] result in
            ...
            case .success(let messages):
                ...
                DispatchQueue.main.async {
                    self?.messagesCollectionView.reloadDataAndKeepOffset() // .reloadDataAndKeepOffset() => fix loading messages
                    
                    if shouldScrollToBottom {
                        //                        if messages.count > 0 {
                        //                            self?.messagesCollectionView.scrollToItem(at: IndexPath(item: messages.count - 1, section: 0), at: .bottom, animated: false)
                        //                        }
...
extension ChatViewController: InputBarAccessoryViewDelegate {
    ...
    private func createMessageId() -> String? {
        ...
        let safeCurrentEmail = DatabaseManager.safeEmai(emailAddress: currentUserEmail)
